...
const TaxDocumentManager = ({ formId, onDocumentUploaded, requiredDocuments = [] }) => {
  ...
  const DocumentUploadZone = ({ category, documents }) => {
    ...
    return (
      <div className={`border-2 border-dashed rounded-lg p-6 transition-colors ${
        completionStatus === 'complete' ? 'border-green-300 bg-green-50 dark:border-green-700 dark:bg-green-950' :
        completionStatus === 'uploaded' ? 'border-blue-300 bg-blue-50 dark:border-blue-700 dark:bg-blue-950' :
        completionStatus === 'missing' && category.required ? 'border-red-300 bg-red-50 dark:border-red-700 dark:bg-red-950' :
        'border-gray-300 bg-gray-50 dark:border-gray-700 dark:bg-neutral-900 hover:border-blue-400 dark:hover:border-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900'
      }`}>
        <div className="text-center">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                {category.required && <span className="text-xs text-red-600 dark:text-red-400 font-medium">Required</span>}
              </div>
            </div>
          ...
          {documents.length === 0 ? (
            <div>
              <Upload className="w-8 h-8 text-gray-400 dark:text-gray-300 mx-auto mb-2" />
            ...
            <div className="space-y-3">
              ...
              <button
                onClick={() => fileInputRef.current?.click()}
                className="text-sm text-blue-600 dark:text-blue-300 hover:text-blue-800 dark:hover:text-blue-400 flex items-center gap-1 mx-auto"
              >
  ...
  const DocumentCard = ({ document, onDelete, onPreview }) => {
    ...
    return (
      <div className="flex items-center justify-between p-3 bg-white dark:bg-neutral-900 border border-gray-200 dark:border-gray-700 rounded-lg">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-blue-100 dark:bg-blue-900 rounded">
            <FileText className="w-4 h-4 text-blue-600 dark:text-blue-300" />
          </div>
          <div>
            <p className="font-medium text-gray-900 dark:text-gray-100 text-sm">{document.name}</p>
            ...
            {isProcessing && (
              <div className="text-xs text-blue-600 dark:text-blue-300 flex items-center gap-1 mt-1">
                <div className="animate-spin rounded-full h-3 w-3 border-b-2 border-blue-600 dark:border-blue-300"></div>
                Processing...
              </div>
            )}
          </div>
  ...
  const DocumentPreviewModal = ({ document, onClose }) => {
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div className="bg-white dark:bg-neutral-900 rounded-xl max-w-4xl w-full max-h-[90vh] flex flex-col">
          <div className="p-4 border-b border-gray-200 dark:border-gray-700">
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100">{document.name}</h2>
              <button onClick={onClose} className="text-gray-400 dark:text-gray-300 hover:text-gray-600 dark:hover:text-gray-100 text-xl">×</button>
            </div>
          </div>
          ...
          {document.ocrData && (
            <div className="p-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-neutral-800">
              <h3 className="font-medium text-gray-900 dark:text-gray-100 mb-2">Extracted Information</h3>
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100 mb-2">Document Manager</h1>
        <p className="text-gray-600 dark:text-gray-400">Upload and manage your tax filing documents with AI-powered data extraction</p>
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-white dark:bg-neutral-900 border border-gray-200 dark:border-gray-700 rounded-lg p-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Required Documents</span>
              <span className="text-sm font-bold text-gray-900 dark:text-gray-100">{Math.round(progress.required)}%</span>
            </div>
          ...
          <div className="bg-white dark:bg-neutral-900 border border-gray-200 dark:border-gray-700 rounded-lg p-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-gray-700 dark:text-gray-300">All Documents</span>
              <span className="text-sm font-bold text-gray-900 dark:text-gray-100">{Math.round(progress.overall)}%</span>
            </div>
            <div className="w-full bg-gray-200 dark:bg-neutral-800 rounded-full h-2">
              <div 
      ...
      {uploadedDocuments.length > 0 && (
        <div className="bg-white dark:bg-neutral-900 border border-gray-200 dark:border-gray-700 rounded-lg p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100">Document Summary</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="text-center p-4 bg-blue-50 dark:bg-blue-900 rounded-lg">
              <div className="text-2xl font-bold text-blue-600 dark:text-blue-300">{uploadedDocuments.length}</div>
            ...
            <div className="text-center p-4 bg-green-50 dark:bg-green-900 rounded-lg">
              ...
              <div className="text-sm text-green-800 dark:text-green-200">Data Extracted</div>
            </div>
            <div className="text-center p-4 bg-purple-50 dark:bg-purple-900 rounded-lg">
              <div className="text-2xl font-bold text-purple-600 dark:text-purple-300">
                {documentCategories.filter(cat => getCategoryCompletionStatus(cat) === 'complete').length}
              </div>
              <div className="text-sm text-purple-800 dark:text-purple-200">Categories Complete</div>
            </div>
          </div>
        </div>
      )}
